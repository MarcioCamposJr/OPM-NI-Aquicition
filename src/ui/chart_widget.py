...
from src.ui.styles import CHANNEL_COLORS, BG_DARKEST, BG_CARD, BORDER, TEXT_SECONDARY


class ChartWidget(QWidget):
    """Grid of 24 real-time PlotItems (6 rows × 4 columns).
    ...
    def _configure_plot(self, plot: pg.PlotItem, channel_index: int) -> None:
        """Style a single plot widget."""
        label = f"CH-{channel_index + 1:02d}"
        plot.setTitle(label, color=TEXT_SECONDARY, size="9pt")
        ...
        ax_left.setStyle(showValues=(col == 0))
        ax_bottom.setStyle(showValues=(row == self._rows - 1))

        if col == 0:
            ax_left.setLabel("mV", color=TEXT_SECONDARY)
        if row == self._rows - 1:
            ax_bottom.setLabel("s", color=TEXT_SECONDARY)
